...
targets.T =(1-par.tau).*W_fc.*grid.N;% +(1-par.tau)*R_fc*grid.K+(1-par.tau).*Profits_fc;
...
% indexes of 05,25,50,90,99
hpick=ceil(mpar.nh/2);
jdh=squeeze(joint_distr(:,:,hpick));
mplusk=mesh.k(:)+mesh.m(:);

[wealth_sort, IX]  = sort(mplusk(:));

wealth_pdf=jdh(IX)/sum(jdh(:));

wpctpicks=[0.05,0.25,0.5,0.9,0.99]';

wpctind=zeros(size(wpctpicks,1),1);

for i=1:size(wpctpicks,1)

    [x,wpctind(i)]=min(abs(cumsum(wealth_pdf)-wpctpicks(i)));
    
    wpctind(i)=IX(wpctind(i))+mpar.nk*mpar.nm*(hpick-1);
end
