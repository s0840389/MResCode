function [c_a_star,m_a_star,k_a_star,c_n_star,m_n_star]=EGM_policyupdate(EVm,EVk,Qminus,PIminus,RBminus,inc,mesh3D,grid,par,mpar)
...
m_star_n = (c_new + mesh3D.m - inc.labor - inc.rent);
m_star_n = (m_star_n<0).*m_star_n./((RBminus+par.borrwedge)/PIminus)...
    + (m_star_n>=0).*m_star_n./(RBminus/PIminus);

% Identify binding constraints
binding_constraints = mesh3D.m < repmat(m_star_n(1,:,:),[mpar.nm 1 1]);
...
E_return_diff = term1./Qminus-EMU; % euquatin s8 -equation s10 =0 econometrica appendix
