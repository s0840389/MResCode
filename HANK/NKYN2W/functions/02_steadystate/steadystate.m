function [c_n_guess,m_n_star,c_a_guess,m_a_star,cap_a_star,psi_guess,joint_distr,R_fc,Wy_fc,We_fc,Profits_fc,Output,grid,par,qs]=steadystate(P_H,grid,meshes,mpar,par)
...
[ N,R_fc,Wy_fc,We_fc,Profits_fc,WW,RR,RBRB,Y,qs,par,ly,le,Mg ] = factor_returns(meshes,grid,par,mpar);
...
[excess,c_n_guess,m_n_star,c_a_guess,m_a_star,cap_a_star,psi_guess,joint_distr, R_fc,Wy_fc,We_fc,Profits_fc,Output,N,qs,par,ly,le,Mg]=excessK(Kcand,c_a_guess,c_n_guess,psi_guess,joint_distr, grid,P_H,mpar,par,meshes);
...
grid.le=le;
grid.Mg=Mg;
grid.ki=grid.K/grid.Mg;
end

function [excess,c_n_guess,m_n_star,c_a_guess,m_a_star,cap_a_star,psi_guess,joint_distr, R_fc,Wy_fc,We_fc,Profits_fc,Output,N,qs,par,ly,le,Mg]=excessK(K,c_a_guess,c_n_guess,psi_guess,joint_distr, grid,P_H,mpar,par,meshes)

grid.K=K;

[ N,R_fc,Wy_fc,We_fc,Profits_fc,WW,RR,RBRB,Output,qs,par,ly,le,Mg ]  = factor_returns(meshes,grid,par,mpar);
...
%setting measure of types of workers
joint_distr(:,:,1:mpar.nhy)=abs(joint_distr(:,:,1:mpar.nhy))/sum(sum(sum(abs(joint_distr(:,:,1:mpar.nhy)))))*mpar.MeasY;
joint_distr(:,:,mpar.nhy+1:end)=abs(joint_distr(:,:,mpar.nhy+1:end))/sum(sum(sum(abs(joint_distr(:,:,mpar.nhy+1:end)))))*mpar.MeasE;
