...
function [ys,check] = DynamicsNKYN_steadystate(ys,exe);
global M_
...
% (9) wages
  ...
  Inv=log((ddelta+ctrend)*exp(k));
    ...
    divy = exp(N)* (exp(y)*exp(mc)	-	exp(w)*exp(ly)	- exp(k)*pk);
	divn = exp(N) *exp(y)*(1-exp(mc)) - exp(w)*exp(le);
