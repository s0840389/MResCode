...
zn tthetan profshr labshr shareE lyle_ratio ;
...
intstar=(1+ctrend)^ssigma*(1+pitstar)/(bbeta)-1; % r*
 ...
 eeps=1/(profshr+shareE*labshr); % match profit share
 
 mu_p=eeps/(eeps-1); % price markup

 lyle_ratio=shareE/(1-shareE);
 
 scaley=mu_p*labshr/(1+lyle_ratio); % thetay*(1-alphay)
 
 tthetan=1/(1-1/mu_p)*labshr/(1+1/lyle_ratio); % thetan*(1-alphan)
    
 aalpha=1-scaley;
 
  zn=1/shareE^tthetan; % expansionary technology
